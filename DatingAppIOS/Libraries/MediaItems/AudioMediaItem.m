...
//		[imageView addSubview:iconView];
	...
	if ((self.status == STATUS_SUCCEED) && (self.cachedAudioImageView == nil))
	{
		...
		UIColor *colorContent = outgoing ? [UIColor whiteColor] : COLOR_OUTGOING;

		UIImage *icon = [[UIImage imageNamed:@"play.png"] jsq_imageMaskedWithColor:colorContent];
		UIImageView *iconView = [[UIImageView alloc] initWithImage:icon];
//		CGFloat ypos = (size.height - icon.size.height) / 2;
//		CGFloat xpos = outgoing ? ypos : ypos + 6;
		iconView.frame = outgoing ? CGRectMake(32, 20, 46, 54) : CGRectMake(35, 20, 46, 54);

		CGRect frame = outgoing ? CGRectMake(15, 75, 60, 20) : CGRectMake(20, 75, 60, 20);
