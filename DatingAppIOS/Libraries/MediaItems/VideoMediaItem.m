...
- (instancetype)initWithFileURL:(NSURL *)fileURL Readable:(NSString *)readable
{
	self = [super init];
	if (self)
	{
...
- (void)setImage:(UIImage *)image
{
    ...
    _image = outgoing ? [image copy] : [[UIImage imageNamed:@"photo.png"] copy];
    if ([self.readable isEqualToString:@"NO"]) {
        _image = outgoing ? [image copy] : [[UIImage imageNamed:@"no_show.png"] copy];
    }else if ([self.readable isEqualToString:@"2"]) {
        _image = outgoing ? [image copy] : [[UIImage imageNamed:@"show2.png"] copy];
    }else if ([self.readable isEqualToString:@"1"]) {
        _image = outgoing ? [image copy] : [[UIImage imageNamed:@"show3.png"] copy];
    }
	_cachedVideoImageView = nil;
}
...
//		[imageView addSubview:iconView];
        if ([self.readable isEqualToString:@"NO"] && !outgoing) {
            imageView.image = [UIImage imageNamed:@"no_show.png"];
