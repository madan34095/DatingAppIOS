...
- (instancetype)initWithImage:(UIImage *)image Width:(NSNumber *)width Height:(NSNumber *)height Readable:(BOOL )readable
{
	self = [super init];
	if (self)
	{
        ...
        self.appliesMediaViewMaskAsOutgoing = readable;
	}
	return self;
}
...
- (UIView *)mediaView
{
	...
	if ((self.status == 2) && (self.cachedImageView == nil))
	{
		[JSQMessagesMediaViewBubbleImageMasker applyBubbleImageMaskToMediaView:imageView isOutgoing:NO];
		self.cachedImageView = imageView;
	}
	if ((self.status == 3) && (self.cachedImageView == nil))
	{
		[JSQMessagesMediaViewBubbleImageMasker applyBubbleImageMaskToMediaView:imageView isOutgoing:YES];
		self.cachedImageView = imageView;
	}
...
{
	PhotoMediaItem *copy = [[[self class] allocWithZone:zone] initWithImage:self.image Width:self.width Height:self.height Readable:@"NO"];
