...
class TinderCard: UIView {
    ...
    func setupView() {
        ...
        statusImageView = UIImageView(frame: CGRect(x: 100 - 37.5, y: 25, width: 65, height: 65))
    ...
    func shakeAnimationCard(completion: @escaping (Bool) -> ()){
        ...
        statusImageView.image = makeImage(name: "ic_unlike")
    ...
    fileprivate func setInitialLayoutStatus(isleft:Bool){
        ...
        statusImageView.image = makeImage(name: isleft ?  "ic_unlike" : "ic_like")
...
extension TinderCard: UIGestureRecognizerDelegate {
    ...
    fileprivate func updateOverlay(_ distance: CGFloat) {
        ...
        statusImageView.image = makeImage(name:  distance > 0 ? "ic_like" : "ic_unlike")
        ...
        if distance > 0 {
            self.statusImageView.frame = CGRect(x: 10, y: 30, width: self.statusImageView.frame.size.width, height: self.statusImageView.frame.size.height)
        }else {
            statusImageView.frame = CGRect(x: self.frame.size.width - 75 + distance/3.7, y: 30, width: statusImageView.frame.size.width, height: statusImageView.frame.size.height)
        }
