...
#import "ScreenshotReportViewCell.h"

#define BUTTON_MENU_HEIGHT      216
#define VOICE_RECORDER_HEIGHT   216
...
@implementation JSQMessagesViewController{
    MediaButtonMenuView *bottomMenu;
    VoiceRecordToolView *voiceRecorderView;
    BOOL isTextMsg;
    BOOL isBottomAreaLock;
}
...
- (void)jsq_configureMessagesViewController
{
    ...
    self.reportCellIdentifier = [ScreenshotReportViewCell cellReuseIdentifier];
    ...
    [self.inputToolbar.contentView.rightBarButtonItem setImage:[UIImage imageNamed:@"ic_message_send_0"] forState:UIControlStateNormal];
...
- (void)viewDidLoad
{
    ...
    //--- start from wsc
    self.bottomToolBarHeight.constant = 0;
    bottomMenu = [[MediaButtonMenuView alloc] initWithFrame:CGRectMake(0,0,SCR_WIDTH,SCALE_X(BUTTON_MENU_HEIGHT))];
    voiceRecorderView = [[VoiceRecordToolView alloc] initWithFrame:CGRectMake(0, 0, SCR_WIDTH, VOICE_RECORDER_HEIGHT)];
    bottomMenu.delegate = self;
    voiceRecorderView.delegate = self;
    handle_tap(self.collectionView, @selector(onCollectionViewClicked));
    UIButton *backBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    UIImage *backBtnImage = [UIImage imageNamed:@"btn_header_back"]  ;
...
//    [self.navigationItem.leftBarButtonItem setImage:[UIImage imageNamed:@"btn_msg_send"]];
}
...
- (IBAction)reportButtonClicked:(id)sender {
    [self didPressBackButton:sender];
}
- (IBAction)blockButtonClicked:(id)sender {
    [self didBlockBackButton:sender];
}
- (IBAction)userImgBtnClicked:(id)sender {
    [self didUserImgButton:sender];
}

-(void)audioRecordFinished:(NSString *)path{
    NSLog(@"audioRecordFinished %@",path);
}
...
- (void) didSeledtedImgFromGallery:(PHAsset *)imgData {
...
//        [self.collectionView scrollRectToVisible:CGRectMake(0.0, 1.0f, 1.0f, 1.0f)
//                                        animated:animated];
//        return;
//    }
//
...
#pragma mark - Messages view controller
- (void)didBlockBackButton:(UIButton *)button
{
    
}

- (void)didUserImgButton:(UIButton *)button
{
    
}

- (void)didPressBackButton:(UIButton *)button
{
...
//    if (!isMediaMessage) {
//        NSString *type_text = [messageItem senderDisplayName];
//        if ([type_text isEqualToString:@"screen_shot"]) {
//            if (!isOutgoingMessage) {
//                cellIdentifier = self.reportCellIdentifier;
//                JSQMessagesCollectionViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:cellIdentifier forIndexPath:indexPath];
//                cell.delegate = collectionView;
//                cell.reportText.text = [messageItem text];
//                cell.cellTopLabel.attributedText = [collectionView.dataSource collectionView:collectionView attributedTextForCellTopLabelAtIndexPath:indexPath];
//                NSString *str = cell.cellTopLabel.text;
//                if(str!=nil){
//                    cell.dateBottomSpace.constant = 15;
//                    cell.cellTopLabel.textColor = [UIColor whiteColor];
//                    float w = 32, h=25;
//                    CGRect frame = CGRectMake(SCR_WIDTH/2-w - 5, 0, w*2, h);
//                    UIView *backView = [[UIView alloc] initWithFrame:frame];
//                    backView.layer.cornerRadius = h/2;
//                    backView.backgroundColor = [UIColor colorWithHexString:@"#949d9c"];
//                    [cell.cellTopLabel addSubview:backView];
//                    
...
//                    [cell.cellTopLabel addSubview:textView];
//                }else{
//                    cell.dateBottomSpace.constant = 0;
//                }
//                return cell;
//                //return screen_shot_cell;
//            }
//        }
//    }
    JSQMessagesCollectionViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:cellIdentifier forIndexPath:indexPath];
...
//        cell.dateBottomSpace.constant = 15;
//        cell.cellTopLabel.textColor = [UIColor whiteColor];
//        cell.messageBubbleTopLabel.frame = CGRectMake(0,0, 100,100);
//
//        float w = 32, h=25;
//        CGRect frame = CGRectMake(SCR_WIDTH/2-w - 5, 0, w*2, h);
//        UIView *backView = [[UIView alloc] initWithFrame:frame];
...
- (void)onCollectionViewClicked{
    self.bottomToolBarHeight.constant = 0.0f;
...
//        [voiceRecorderView removeFromSuperview];
        [self.bottonToolBarView addSubview:bottomMenu];

        self.bottomToolBarHeight.constant = SCALE_X(BUTTON_MENU_HEIGHT);
        [UIView animateWithDuration:0.3 animations:^{
            [self.view setNeedsUpdateConstraints];
            [self.view layoutIfNeeded];
            [self jsq_updateCollectionViewInsets];
        ...
        isBottomAreaLock = YES;
        [self.view endEditing:YES];

        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0), ^{
            [NSThread sleepForTimeInterval:0.7];
            self->isBottomAreaLock = NO;
        });
...
- (void)messagesInputToolbar:(JSQMessagesInputToolbar *)toolbar didPressRightBarButton:(UIButton *)sender
{
    if(isTextMsg==YES){
        ...
        [self.bottonToolBarView addSubview:voiceRecorderView];

        self.bottomToolBarHeight.constant = VOICE_RECORDER_HEIGHT;
        [UIView animateWithDuration:0.3 animations:^{
            [self.view setNeedsUpdateConstraints];
            [self.view layoutIfNeeded];
            [self jsq_updateCollectionViewInsets];
...
#pragma mark - MediaButtonMenuViewDelegate
...
- (void)textViewDidChange:(UITextView *)textView
{
    ...
    if([self.inputToolbar.contentView.textView hasText]){
        [self.inputToolbar.contentView.rightBarButtonItem setImage:[UIImage imageNamed:@"ic_message_send_1"] forState:UIControlStateNormal];
        isTextMsg = YES;
    }
    else{
        [self.inputToolbar.contentView.rightBarButtonItem setImage:[UIImage imageNamed:@"ic_message_send_0"] forState:UIControlStateNormal];
...
{
    if (![self.inputToolbar.contentView.textView isFirstResponder] && self.bottomToolBarHeight.constant == 0.0f) {
        return;
    }
...
- (void)jsq_setToolbarBottomLayoutGuideConstant:(CGFloat)constant
{
    if(isBottomAreaLock==YES) return;
    
    //self.toolbarBottomLayoutGuide.constant = constant;
    self.bottomToolBarHeight.constant = constant;
